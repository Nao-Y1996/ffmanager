...
all_users.each do |user|
  if userID <= 9 # 最初の九人は部門長
    ...
    UserGenreInfo.create!(genre_id: genre2, user_id: userID, priority: 2, is_valid: true, is_genre_leader: false)
  elsif userID >= 100 # 1年生は1ジャンを登録申請状態で作成する
    User.create!(name: user[0], name_kana: user[1], email: user[2], term: user[3], student_number: user[4],
               is_active: true, is_admin: false,  password: "password", password_confirmation: "password")
    genre1 = rand(1..9)
    UserGenreInfo.create!(genre_id: genre1, user_id: userID, priority: 1, is_valid: false, is_genre_leader: false)
  else
    User.create!(name: user[0], name_kana: user[1], email: user[2], term: user[3], student_number: user[4],
               is_active: true, is_admin: false,  password: "password", password_confirmation: "password")
    genre1 = rand(1..9)
...
for eventID in 1..Event.all.length do
  ...
  for i in 1..rand(1..3) do
    begin
       ...
       EventParticipation.create!(user_id: rand(1..99),event_id: eventID, is_event_admin: true)
    rescue Exception => e
    end
  end
  #参加者の作成
  for i in 1..rand(15..30) do
    begin
       EventParticipation.create!(user_id: rand(1..99),event_id: eventID, is_event_admin: false)
    rescue Exception => e
    end
  end
end
