...
for j in 1..9 do
  for i in start..finish do
    student_num = i+1000
    User.create!(name: "てすとゆーざー#{i}", name_kana: "テストユーザー#{i}", term: term(18), student_number: student_num, is_active: true,
                 is_admin: false, email: student_num.to_s+"@example.com", password: "password", password_confirmation: "password")
...
3.times do |i|
	student_num = i+2000
  User.create!(name: "代表#{i}", name_kana: "ダイヒョウ#{i}", term: 16, student_number: student_num, is_active: true,
